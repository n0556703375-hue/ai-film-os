...
class ContinuityIssueUpdateProjectIsolationTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_update_rejects_issue_owned_by_another_project_without_mutation(self):
        updated = issues.update_issue(
            self.issue["id"],
            self.second["project"]["id"],
            {"message": "Cross-project mutation"},
        )

        self.assertIsNone(updated)
        self.assertEqual(self._stored_issue()["message"], "Original message")

    def test_update_changes_issue_only_in_owning_project(self):
        updated = issues.update_issue(
            self.issue["id"],
        ...
        self.assertEqual(updated["message"], "Updated safely")
        self.assertEqual(updated["project_id"], self.first["project"]["id"])
        self.assertEqual(
            issues.list_issues(project_id=self.second["project"]["id"]),
            [],
        )

    def test_update_rejects_moving_issue_to_another_project(self):
        with self.assertRaisesRegex(ValueError, "לא ניתן להעביר"):
            issues.update_issue(
                self.issue["id"],
                self.first["project"]["id"],
        ...
        self.assertEqual(stored["project_id"], self.first["project"]["id"])
        self.assertEqual(stored["message"], "Original message")

    def test_update_rejects_cross_project_shot_relationship(self):
        ...
        self.assertEqual(self._stored_issue()["shot_id"], self.first["shot"]["id"])
