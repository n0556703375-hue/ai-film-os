const LOCK_LABELS = {draft: "טיוטה", review: "בבדיקה", locked: "נעולה"};
...
async function openAsset(id) {
  ...
  <div class="row"><h2>${esc(asset.name)}</h2>${isCharacter ? `<span class="badge ${isLocked ? "approved" : ""}">Character Lock: ${esc(lockLabel)}</span>` : ""}</div>
  <div class="workspace-grid"><div class="workspace-section">
  ${isLocked ? `<div class="card"><b>הזהות נעולה</b><p class="meta">שדות הזהות והרפרנס הראשי מוגנים משינוי עד לפתיחת הנעילה.</p></div>` : ""}
  ...
  ${isCharacter ? `<div class="workspace-section"><div class="section-toolbar"><h3>Character Lock</h3>${isLocked ? `<button class="danger" onclick="unlockCharacter(${asset.id})">פתיחת נעילה</button>` : ""}</div>
  <p class="meta">יש לאשר רפרנס אחד לפחות, לבחור אותו כ־Master ואז לנעול. רק דמות נעולה תיחשב זהות סופית להפקה.</p>
  ...
  <div class="reference-grid">${references.length ? references.map((r)=>`
    <div class="card ${asset.master_reference_id===r.id ? "approved" : ""}">
      ...
      ${!isLocked && r.approved ? `<button onclick="lockCharacter(${asset.id},${r.id})">נעילה כ־Master</button>` : ""}</div>
...
async function lockCharacter(assetId, referenceId) {
  if (!confirm("לנעול את הדמות עם הרפרנס הזה כ־Master? שדות הזהות יוגנו משינוי.")) return;
  try {
...
async function unlockCharacter(assetId) {
  if (!confirm("לפתוח את נעילת הדמות? לאחר מכן ניתן יהיה לשנות את הזהות והרפרנסים.")) return;
  try {
