...
async function loadQA() {
  ...
  $("qa").innerHTML = `
    <div class="qa-toolbar">
      <button onclick="filterIssues('all')">הכול</button>
      <button class="secondary" onclick="filterIssues('critical')">Critical</button>
      <button class="secondary" onclick="filterIssues('high')">High</button>
      <button class="secondary" onclick="filterIssues('medium')">Medium</button>
      <button class="secondary" onclick="filterIssues('low')">Low</button>
      <button class="secondary" onclick="filterIssues('resolved')">Resolved</button>
      <button class="danger" onclick="clearResolved()">מחיקת בעיות פתורות</button>
    </div>
    <div id="issueGrid" class="grid">
      ${issues.length ? issues.map(issueCard).join("") : "<p>אין עדיין בעיות שמורות. הפעילי בדיקת רציפות על שוט.</p>"}
    </div>`;
}

function issueCard(issue) {
  return `<div class="card issue-${esc(issue.severity)} ${issue.resolved?"issue-resolved":""}"
      data-severity="${esc(issue.severity)}" data-resolved="${issue.resolved}">
    <span class="badge">${esc(issue.severity)}</span>
    <div class="title">${esc(issue.category)}</div>
    <p>${esc(issue.message)}</p>
    <div class="meta">
      ${issue.shot_number ? `שוט ${issue.shot_number}: ${esc(issue.shot_title)}` : ""}
      ${issue.asset_name ? ` · ${esc(issue.asset_name)}` : ""}
    </div>
    <div class="row">
      ${issue.shot_id ? `<button onclick="openShot(${issue.shot_id})">פתיחת השוט</button>` : ""}
      <button class="secondary" onclick="toggleIssue(${issue.id},${!issue.resolved})">
        ${issue.resolved ? "פתיחה מחדש" : "סימון כפתור"}
      </button>
    </div>
  </div>`;
}
...
function filterIssues(filter) {
  document.querySelectorAll("#issueGrid .card").forEach((card)=>{
    const visible =
      filter==="all" ||
      (filter==="resolved" && card.dataset.resolved==="1") ||
      card.dataset.severity===filter;
    card.style.display = visible ? "" : "none";
  });
}

async function toggleIssue(id, resolved) {
  await api(`/api/issues/${id}/resolve?resolved=${resolved}`, {method:"PATCH"});
  await loadQA();
  await loadDashboard();
}

async function clearResolved() {
  if (!confirm("למחוק לצמיתות את כל הבעיות שסומנו כפתורות?")) return;
  await api("/api/issues/resolved", {method:"DELETE"});
  await loadQA();
}
